    ...
    <div className="flex min-h-screen flex-col bg-gray-50 p-4">
      ...
      <div className="rounded-lg bg-white p-6 shadow-md">
        ...
        <p className="text-gray-700">
          ...
          {new Date(book.publishedAt as Date).toLocaleDateString()}
        </p>
        <p className="text-gray-700">
          ...
          <ul className="space-y-4">
            {book.reviews.map((review) => (
              <li key={review.id} className="border-b pb-4">
                ...
                {/* <p className="text-gray-700">
                  <strong>Score:</strong> {book.ratings.reduce( (acc, curr) => acc + curr.score, 0)} / 5
                </p> */}
      ...
      {isReviewModalOpen && (
        <Modal onClose={() => setIsReviewModalOpen(false)}>
          <ReviewForm
            bookId={book.id}
            onSubmit={(data) => createReviewMutation.mutate({...data, bookId: data.bookId, userId: session?.user.id as string})}
      ...
      {isRatingModalOpen && (
        <Modal onClose={() => setIsRatingModalOpen(false)}>
          <RatingForm
            bookId={book.id}
            onSubmit={(data) => createRatingMutation.mutate({...data, bookId: data.bookId, rating: data.score, userId: session?.user.id as string })}
